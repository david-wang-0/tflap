...
/* June 2016                                            */
/********************************************************/
/* State (frontier state) and POP linearization  .      */
/********************************************************/

//#define DEBUG_STATE_ON
...
void Linearizer::setCurrentBasePlan(Plan* plan) {
	iteration++;
	basePlan = plan;
	if (iteration == MAX_UNSIGNED_INT) {	// Reset matrix (maximum number of iterations reached)
		iteration = 1;
...
void Linearizer::computeBasePlanSubcomponents(Plan* base) {
	if (base == nullptr) basePlanComponents.clear();
	else {
...
void Linearizer::computeOrderMatrix() {
	...
	for (unsigned int i = 0; i < basePlanComponents.size(); i++) {
		if (basePlanComponents[i]->action != nullptr) {
			matrix[stepToStartPoint(i)][stepToEndPoint(i)] = iteration;		// Start point of the step always before the end point of the step 
...
unsigned int Linearizer::topologicalOrder(TTimePoint orig, vector<TTimePoint>* linearOrder, unsigned int pos, vector<bool>* visited) {
	(*visited)[orig] = true;
...
TState* Linearizer::getFrontierState(SASTask* task, LandmarkHeuristic* hLand) { //, double* timeNewStep) {
	unsigned int numActions = basePlanComponents.size();
	if (plan != nullptr) numActions++;
...
string Linearizer::planToPDDL(Plan* p, SASTask* task) {
	...
	if (state != nullptr) {
		for (unsigned int i = 1; i < last; i++) {
			SASAction* a = i < basePlanComponents.size() ? basePlanComponents[i]->action : plan->action;
    ...
    for(unsigned int i = 0; i < linearSteps.size(); i++) {
		...
		oss << s.time << ": (" << s.actionName << ") [" << s.duration << "]" << endl;
    }
	oss << ";Makespan: " << (int)makespan << endl;
	oss << ";Actions:  " << numPlanActions << endl;
	return oss.str();
}
...
TState* Linearizer::linearize(unsigned int numActions, unsigned int numTimeSteps, SASTask* task, LandmarkHeuristic* hLand) {
	bool invalidPlan = false;
	vector<TTimePoint> linearOrder(numTimeSteps);
	SASAction* lastAction = plan != nullptr ? plan->action : basePlanComponents.back()->action;
	...
	while (repeat) {
		if (!isSolution) fixScheduledTimes(numTimeSteps, state, &repeat, &invalidPlan, &linearOrder);
		else fixScheduledTimesForGoal(numTimeSteps, state, &repeat, &invalidPlan, &linearOrder);
		if (invalidPlan) {
			delete state;
			break;
		}
	...
	if (!invalidPlan) {
		...
		for (unsigned int i = numTimeSteps - 1; i >= 2; i--) {
			if (time[i] > makespan) {
				TStep step = i >> 1;
				Plan* p = step < basePlanComponents.size() ? basePlanComponents[step] : plan;
				if (!p->action->isTIL) makespan = time[i];
			}
		}
...
void Linearizer::fixScheduledTimes(unsigned int numTimeSteps, TState* state, bool* repeat,
		bool* invalidPlan, std::vector<TTimePoint>* linearOrder) {
	*repeat = false;
	pq.clear();
	if (plan != nullptr) plan->unsatisfiedNumericConditions = false;
	for (unsigned int i = 2; i < numTimeSteps; i++) {
		TStep step = i >> 1;
		Plan* p = step < basePlanComponents.size() ? basePlanComponents[step] : plan;
		if (p->fixedEnd >= 0) {
			if (((i & 1) == 0 && time[i] >= 1.5*EPSILON) ||
				...
				for (unsigned int j = 1; j <= pq.size(); j++) delete (ScheduledPoint*)pq.at(j);
				pq.clear();
				*invalidPlan = true;
				return;
			}
	...
	while (pq.size() > 0) {
		...
		start = (tp & 1) == 0;
		dur = duration[tp >> 1];
		//cout << tp << " -> " << p->a->name << " -> " << p->time << endl;
		if (start) {	// Start time point of an action
			if (time[tp + 1] - time[tp] >= dur + EPSILON) {
				//	cout << "Delay: action " << i << ": " << time[start] << " = " << time[start + 1] << " - " << duration[i] << endl;
	...
	if (!unsatisfiedNumCond.empty() && plan != nullptr) plan->unsatisfiedNumericConditions = true;
}

void Linearizer::fixScheduledTimesForGoal(unsigned int numTimeSteps, TState* state, bool* repeat,
		bool* invalidPlan, std::vector<TTimePoint>* linearOrder) {
	/*
	cout << "Linear order: [";
	...
	*repeat = false;
	...
	while (pq.size() > 0) {
		...
		if (start) {	// Start time point of an action
			if (time[tp + 1] - time[tp] >= dur + EPSILON) {
				//cout << "Delay: action " << tp << ": " << time[start] << " = " << time[start + 1] << " - " << dur << endl;
				if (p->plan->fixedEnd < 0) {
					time[tp] = time[tp + 1] - dur;
					*invalidPlan = checkTopologicalOrder(linearOrder);
				} else {
		...
		if (abs(p->time - currentTime) < EPSILON/2) {
			...
			for (unsigned int i = 0; i < sameTime.size(); i++) {
				if (checkNumericMutex(tp, a, sameTime[i]->p, sameTime[i]->plan->action)) {
					if (start) {	// Start point of the action must be delayed
						time[tp] += EPSILON;
						...
						break;
					} else {		// End point must be delayed.
						time[tp - 1] += EPSILON;
						for (unsigned int j = 2; j < numTimeSteps; j++) {
							if (existOrder(tp - 1, j)) time[j] += EPSILON;
						}
			...
			currentTime = p->time;
			for (unsigned int i = 0; i < sameTime.size(); i++) delete sameTime[i];
			sameTime.clear();
		}
		if (!checkNumericConditions(tp, a, state, dur)) {
			unsatisfiedNumCond.push_back(tp);
...
void Linearizer::checkUnsatisfiedConditions(double currentTime, TState* state, std::vector<TTimePoint>* unsatisfiedNumCond) {
	unsigned int i = 0;
...
void Linearizer::initialPlanSchedule(std::vector<TTimePoint>* linearOrder, unsigned int numTimeSteps) {
	...
	for (i = 0; i < numNumVars; i++) numState[i] = initialState->numState[i];
	for (i = 2; i <= numTimeSteps; i++) {
		...
		for (j = i + 1; j <= numTimeSteps; j++) {
			p2 = (*linearOrder)[j];
			if (existOrder(p1, p2)) {
				if (startPoint && p2 == p1 + 1) {	// p1 and p2 are the start and the end of the same action, respectively
...
bool Linearizer::checkTopologicalOrder(vector<TTimePoint>* linearOrder) {
	for (unsigned int i = 2; i < linearOrder->size(); i++) {
		TTimePoint p = (*linearOrder)[i];
		unsigned int prev = i - 1;
		if (time[p] < time[(*linearOrder)[prev]]) {	// Wrong order, p should be before linearOrder[prev]
			TTimePoint p2 = (*linearOrder)[prev];
			do {
				//if (this->plan == nullptr || this->plan->id == PLAN_DEB)
				//	cout << "Wrong order: " << p2 << " -> " << p << endl;
				if (existOrder(p2, p)) {	// Invalid plan: doesn’t meet temporal contraints
					//if (this->plan == nullptr || this->plan->id == PLAN_DEB) cout << "Invalid plan: doesn’t meet temporal contraints" << endl;
					return true;
				}
...
bool Linearizer::checkActionDelayNeeded() {
	...
	for (unsigned int i = 1; i < n; i++) {
		if (!basePlanComponents[i]->action->startNumEff.empty() || !basePlanComponents[i]->action->endNumEff.empty())
			numEffects = true;
		start = i << 1;
		if (time[start + 1] - time[start] >= duration[i] + EPSILON) {
			//	cout << "Delay: action " << i << ": " << time[start] << " = " << time[start + 1] << " - " << duration[i] << endl;
			time[start] = time[start + 1] - duration[i];
			delay = true;
		}
	}
	if (!delay && numEffects) {
...
bool Linearizer::schedulePlan(vector<TTimePoint>* linearOrder, unsigned int numTimeSteps, TState* state, LandmarkHeuristic* hLand) {
	unsigned int i, j;
	makespan = 0;
	numTimeSteps--;
	TTimePoint p1, p2;
	...
	for (i = 2; i <= numTimeSteps; i++) {
		...
		for (j = i + 1; j <= numTimeSteps; j++) {
			p2 = (*linearOrder)[j];
			if (existOrder(p1, p2)) {
				//if (this->plan == nullptr || this->plan->id == PLAN_DEB) cout << "* " << p1 << " --> " << p2 << endl;
				if ((p1 & 1) == 0 && p2 == p1 + 1) {	// p1 and p2 are the start and the end of the same action, respectively
					this->duration[step1] = dur;
					time[p2] = time[p1] + dur; //computeActionDuration(p1 >> 1, state);

					//if (this->plan == nullptr || this->plan->id == PLAN_DEB)
					//	cout << "Time[" << p2 << "] = " << "Time[" << p1 << +"] + " << computeActionDuration(p1 >> 1, state) << " = " << time[p2] << endl;
				}
				else {
					//if (this->plan == nullptr || this->plan->id == PLAN_DEB)
					...
					if (time[p2] < time[p1] + EPSILON) {
						...
						//if (this->plan == nullptr || this->plan->id == PLAN_DEB)
						//	cout << "Time[" << p2 << "] = " << "Time[" << p1 << +"] + " << EPSILON << " = " << time[p2] << endl;
					}
				}
			}
		}
...
bool Linearizer::checkNumericMutex(TTimePoint p1, SASAction* a1, TTimePoint p2, SASAction* a2) {
	TVariable v;
	if ((p1 & 1) == 0) {	// Start of a1
		if ((p2 & 1) == 0) {	// Start of a2
			// Start effect of a1 with start effect of a2
			for (unsigned int i = 0; i < a1->startNumEff.size(); i++) {
				v = a1->startNumEff[i].var;
				for (unsigned int j = 0; j < a2->startNumEff.size(); j++) {
					if (a2->startNumEff[j].var == v) {
						return true;
					}
				}
			}
			...
			for (unsigned int i = 0; i < a1->startNumEff.size(); i++) {
				v = a1->startNumEff[i].var;
				for (unsigned int j = 0; j < a2->endNumEff.size(); j++) {
					if (a2->endNumEff[j].var == v) {
						return true;
					}
				}
			}
		...
		if ((p2 & 1) == 0) {	// Start of a2
			...
			for (unsigned int i = 0; i < a1->endNumEff.size(); i++) {
				v = a1->endNumEff[i].var;
				for (unsigned int j = 0; j < a2->startNumEff.size(); j++) {
					if (a2->startNumEff[j].var == v) {
						return true;
					}
...
bool Linearizer::checkNumericMutexWithStartPoint(TTimePoint p, TTimePoint prev, SASAction* a) {
	if (a->startNumEff.empty() && a->endNumEff.empty()) return false;
	TStep prevStep = prev >> 1;
	SASAction* aPrev = prevStep < basePlanComponents.size() ? basePlanComponents[prevStep]->action : plan->action;
	if (aPrev->startNumEff.empty() && aPrev->endNumEff.empty()) return false;
	//if (a->name.compare("board p1 fast1 f6") == 0 && aPrev->name.compare("board p4 fast1 f6") == 0);
	//	cout << "AQUI" << endl;
	TVariable v;
	if ((prev & 1) == 0) {	// Start point together with previous start point
	...
	} else {	// Start point together with previous end point
		for (unsigned int i = 0; i < a->startNumEff.size(); i++) {
			v = a->startNumEff[i].var;
			for (unsigned int j = 0; j < aPrev->endNumEff.size(); j++) {
				if (aPrev->endNumEff[j].var == v) {
...
void Linearizer::updateNumState(TTimePoint p, SASAction* a, float* numState, double dur) {
	vector<SASNumericEffect>* numEff = (p & 1) == 0 ? &(a->startNumEff) : &(a->endNumEff);
...
void Linearizer::updateState(TTimePoint p, SASAction* a, TState* state, double dur) {
	...
	vector<SASCondition>* eff;
	vector<SASNumericEffect>* numEff;
	if ((p & 1) == 0) {	// Start point of an action
		eff = &(a->startEff);
		numEff = &(a->startNumEff);
	}
	else {				// End point of an action
		eff = &(a->endEff);
		numEff = &(a->endNumEff);
	}
...
bool Linearizer::checkNumericConditions(TTimePoint p, SASAction* a, TState* state, double dur) {
	vector<SASNumericCondition>* numCond = (p & 1) == 0 ? &(a->startNumCond) : &(a->endNumCond);
...
double Linearizer::computeActionDuration(TStep step, TState* state) {
	SASAction* action = step < basePlanComponents.size() ? basePlanComponents[step]->action : plan->action;
