...
Planner::Planner(SASTask* task, Plan* initialPlan, TState* initialState, bool forceAtEndConditions, bool filterRepeatedStates,
		bool generateTrace, vector<SASAction*>* tilActions, Planner* parentPlanner) {
	this->task = task;
...
Plan* Planner::improveSolution(uint16_t bestG, float bestGC, bool first) {
	...
	solution = nullptr;
	while (qualitySelector.size() > 0 && solution == nullptr) {
