...
PlannerReversible::PlannerReversible(SASTask* task, Plan* initialPlan, TState* initialState, bool forceAtEndConditions, bool filterRepeatedStates,
		bool generateTrace, std::vector<SASAction*>* tilActions, Planner* parentPlanner)
	...
	if (successors->informativeLandmarks() || 1.5f * initialPlan->hLand >= initialPlan->h) {	// Landmarks available
		if (tilActions != nullptr) {
			sel->addQueue(SEARCH_G_HFF + SEARCH_PLATEAU);
			sel->addQueue(SEARCH_G_3HLAND);
		} else {
...
Plan* PlannerReversible::plan() {
	...
	while (solution == nullptr && !emptySearchSpace()) {
		searchStep();
		if (generateTrace) {
