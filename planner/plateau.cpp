...
bool Plateau::searchStep(bool concurrent) {
	...
	for (Plan* p : suc) {
		...
		if (p->h < hToImprove) {
			...
			bestPlan = p;
			hToImprove = p->h;
			if (task->numGoalsInPlateau > 1) task->numGoalsInPlateau--;
			//cout << "E" << hToImprove << ".";
		}
	}
...
Plateau::Plateau(SASTask* sTask, Plan* initPlan, Successors* s, float h, int selectorIndex) {
	...
	this->selectorIndex = selectorIndex;
	hToImprove = h;
	selector = new PlateauSelector(SEARCH_G_2HFF + SEARCH_PLATEAU);
	addOpenNodes(initPlan);
	cout << "|" << initPlan->h << "->" << hToImprove << ".";
}
...
bool Plateau::searchStep(bool concurrent) {
	...
	if (concurrent) successors->computeSuccessorsConcurrent(base, &suc);
	else successors->computeSuccessors(base, &suc);
	base->addChildren(suc);
	for (Plan* p : suc) {
		selector->add(p);
