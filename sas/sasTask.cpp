...
void SASVariable::addInitialValue(unsigned int sasValue, bool isTrue, float timePoint) {
	...
	if (index == numeric_limits<unsigned int>::max()) {
		cout << "Invalid intial value " << sasValue << " for variable " << name << endl;
		assert(false);
	}
	if (isTrue) {
		...
		cout << "Unable to translate negated initial value for variable " << name << endl;
		assert(false);
	}
}
...
unsigned int SASVariable::getOppositeValue(unsigned int v) {
	...
	cout << "Invalid value " << v << " for variable " << name << endl;
	assert(false);
	return 0;
}
...
void NumericVariable::addInitialValue(float value, float time) {
	for (unsigned int i = 0; i < this->value.size(); i++) {
		if (time == this->time[i]) {
			...
			cout << "Contradictory value " << value << " in time " << time << " for variable " << name << endl;
			assert(false);
		}
	}
...
string SASNumericExpression::toString(vector<NumericVariable> *numVariables) {
	string s;
	switch (type) {
	...
	case '#':
			if (terms.size() == 0) s = "#t";
			else s = "(* #t " + terms[0].toString(numVariables) + ")";
		break;
	default:  s = "?duration";	
	}
	return s;
...
string SASDuration::toString(vector<NumericVariable> *numVariables) {
	return SASTask::toStringTime(time) + "(" + SASTask::toStringComparator(comp) + 
		" ?duration " + exp.toString(numVariables) + ")";
}
...
string SASNumericCondition::toString(vector<NumericVariable> *numVariables) {
...
string SASNumericEffect::toString(vector<NumericVariable> *numVariables) {
...
// Adds a new action
SASAction* SASTask::createNewAction(string name) {
...
string SASTask::toStringPreference(SASPreference* pref) {
	return "preference " + preferenceNames[pref->index] + " " +
		toStringGoalDescription(&(pref->preference));
}

// Returns a string representation of this goal description
string SASTask::toStringGoalDescription(SASGoalDescription* g) {
...
string SASTask::toStringConstraint(SASConstraint* c) {
	string s = "(";
	switch (c->type) {
	case '&':
		s += "and";
	...
	case 'S':
		...
		break;
	case 'W':
		s += "within " + to_string(c->time[0]) + " " + toStringGoalDescription(&(c->goal[0]));
		break;
	case 'O':
		s += "at-most-once " + toStringGoalDescription(&(c->goal[0]));
		break;
	case 'F':
		s += "sometime-after " + toStringGoalDescription(&(c->goal[0])) + " " + toStringGoalDescription(&(c->goal[1]));
		break;
	case 'B':
		s += "sometime-before " + toStringGoalDescription(&(c->goal[0])) + " " + toStringGoalDescription(&(c->goal[1]));
		break;
	case 'T':
		s += "always-within " + to_string(c->time[0]) + " " + toStringGoalDescription(&(c->goal[0])) + " " + toStringGoalDescription(&(c->goal[1]));
		break;
	case 'D':
		s += "hold-during " + to_string(c->time[0]) + " " + to_string(c->time[1]) + " " + toStringGoalDescription(&(c->goal[0]));
		break;
	case 'H':
		s += "hold-after " + to_string(c->time[0]) + " " + toStringGoalDescription(&(c->goal[0]));
		break;
	}
	return s + ")";
}
